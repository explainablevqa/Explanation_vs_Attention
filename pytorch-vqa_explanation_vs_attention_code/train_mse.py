...
def run(net,net_grad, loader, optimizer,optimizer_grad, tracker, train=False, prefix='', epoch=0):
    ...
    for v, q, a, idx, image_id, q_len in tq:
        ...
        if train:
            ...
            # print('final_att',final_att.shape)
            # print('torch.Tensor(gradients).cuda()',grad_input.shape)
            loss_grad=mse(final_att,grad_input)
...
def main():
    ...
    optimizer_grad = optim.Adam([p for p in net_grad.parameters() if p.requires_grad])
    ...
    for i in range(0, config.epochs):
        _ = run(net,net_grad, train_loader, optimizer,optimizer_grad, tracker, train=True, prefix='train', epoch=i)
        r = run(net,net_grad, val_loader, optimizer, optimizer_grad,tracker, train=False, prefix='val', epoch=i)
